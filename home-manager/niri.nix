...
}:
{
  ...
  systemd.user.services =
    lib.mapVals
      ...
      (
        let
          inherit (config.commands) swaybg waybar;
        in
        {
          inherit waybar;
  ...
  home.packages = [
    pkgs.qt6.qtwayland
    pkgs.wayland
    pkgs.alacritty # in case of non-parsing config
    pkgs.fuzzel
  ];
    ...
    in
    {
      ...
      input = {
        keyboard = {
          xkb = {
            ...
            layout = "us,workman-p";
            options = "caps:escape,compose:sclk";
            # model = "";
            # rules = "";
            # variant = "";
          };
      ...
      binds =
            ...
            let
              ...
              format = prefix: suffix: {
                ...
                value =
                  ...
                  in
                  {
                    action.${replacer "${prefix.action}-${actual-suffix.action}"} = actual-suffix.args;
                  };
              };
              pairs =
                attrs: fn:
        ...
        lib.mapVals (str: { action.spawn = sh str; }) {
          ...
          "Mod+I" = term nmtui;
          "Mod+Shift+I" = "${toggle rmenu} -r network";
          "Mod+U" = term power;
          "Ctrl+Alt+Delete" = term "sudo ${btm}";
          "Mod+L" = swaylock;
          # "Alt+Space" = "${swaync-client} --close-latest";
          "Mod+Escape" = "${swaync-client} --close-all";
          "Mod+Grave" = "${swaync-client} --toggle-panel";
          "Mod+Space" = "${toggle rmenu} -r drun";
          "Mod+J" = "${toggle rmenu} -r drun";
          "Shift+Mod+Space" = term jit;
          "Shift+Mod+J" = term jit;
          "Mod+Alt+Space" = hold "gum input --placeholder 'tldr tldr' | xargs comma";
        }
        //
          lib.mapVals
            (str: {
            ...
            })
            {
              ...
              XF86Tools = "${toggle rmenu} -r audio";
              # screen brightness
        ...
        // (binds {
          # add faux workspace 'zero' for niri's `empty-workspace-above-first`
        ...
        // (lib.mapVals
          (
            attrs:
            let
              tpl = builtins.elemAt (lib.attrsets.attrsToList attrs) 0;
            in
            { action."${tpl.name}" = [ ]; } // tpl.value
          )
          {
            "Mod+WheelScrollDown".focus-workspace-down.cooldown-ms = 150;
            "Mod+WheelScrollUp".focus-workspace-up.cooldown-ms = 150;
            "Mod+Shift+WheelScrollDown".move-column-to-workspace-down.cooldown-ms = 150;
            "Mod+Shift+WheelScrollUp".move-column-to-workspace-up.cooldown-ms = 150;
          }
        );
